...
class AWS:
    def __init__(
        ...
        cf_public_key_id: str,
        cf_private_key: bytes,
    ) -> None:
        ...
        self.s3 = boto3.client(
            ...
            endpoint_url=f"https://s3.{self.s3_region_name}.amazonaws.com",
        ...
        self.mediaconvert = boto3.client(
            "mediaconvert",
            region_name=self.s3_region_name,
            endpoint_url=f"https://mqm13wgra.mediaconvert.{self.s3_region_name}.amazonaws.com",
            aws_access_key_id=access_key_id,
            aws_secret_access_key=secret_access_key,
        )
    ...
    def rsa_sign(self, message):
        ...
        private_key = serialization.load_pem_private_key(
            self.cf_private_key, password=None, backend=default_backend()
        )
    ...
    def _is_invalidation_request_completed(self, invalidation_id: str) -> bool:
        ...
        response = self.cloudfront.get_invalidation(
            DistributionId=self.cf_distribution_id, Id=invalidation_id
        )
        return response["Invalidation"]["Status"] == "Completed"

    def _get_presigned_url(
        self, object_key: str, expiration: datetime.datetime
    ) -> str:
        ...
        cloudfront_signer = CloudFrontSigner(
            self.cf_public_key_id, self.rsa_sign
        )
    ...
    def get_upload_url(
        self, upload_uuid: str, filename: str, expiration_in_hours: int
    ) -> str:
    ...
    def get_thumbnail_url(
        self, upload_uuid: str, expiration_in_hours: int
    ) -> str:
    ...
    def get_presigned_url_post(
        self, upload_uuid: str, filename: str, expiration: int = 3600
    ) -> dict:
        ...
        response = self.s3.generate_presigned_post(
            self.s3_bucket_name, key, ExpiresIn=expiration
        )
        return response
    ...
    def create_mediaconvert_job(self, upload_uuid: str, filename: str) -> str:
        ...
        job_object["Settings"]["Inputs"][0][
            "FileInput"
        ] = f"s3://{self.s3_bucket_name}/uploads/{upload_uuid}/{filename}"
        job_object["Settings"]["OutputGroups"][0]["OutputGroupSettings"][
            "HlsGroupSettings"
        ][
            "Destination"
        ] = f"s3://{self.s3_bucket_name}/uploads/{upload_uuid}/hls/index"
        job_object["Settings"]["OutputGroups"][1]["OutputGroupSettings"][
            "FileGroupSettings"
        ][
            "Destination"
        ] = f"s3://{self.s3_bucket_name}/uploads/{upload_uuid}/thumbnail"
    ...
    def delete_uploads(self, *upload_ids: int) -> None:
        ...
        for upload_id in upload_ids:
            pages = paginator.paginate(
                Bucket=self.s3_bucket_name, Prefix=f"uploads/{upload_id}/"
            )
            ...
            for item in pages.search("Contents"):
                ...
                if len(delete_keys["Objects"]) >= 1000:
                    self.s3.delete_objects(
                        Bucket=self.s3_bucket_name, Delete=delete_keys
                    )
                    delete_keys = dict(Objects=[])
                    found = True
...
def main() -> None:
    ...
    job_id = aws.create_mediaconvert_job(
        "exampleuploaduid", "fullcourtstock.mp4"
    )
    while True:
